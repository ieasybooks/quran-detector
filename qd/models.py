...
class Term:
    ...
    def print(self, spaces=""):
        print(f"{spaces}{self.text} (terminal: {self.terminal}, abs_terminal: {self.abs_terminal})")
...
class MatchRecord:
    ...
    """
    def __init__(self, verse_text: str, sura_name: str, start_idx: int, end_idx: int, errors, start_in_text: int, end_in_text: int):
    ...
    def get_start_index(self, token1: str, token2: str, normalized_original: str) -> int:
    ...
    def get_adjusted(self, start_idx: int, start_term: str, original_tokens: list) -> int:
        while start_idx < len(original_tokens):
            current = original_tokens[start_idx].strip()
            if current == start_term or ('و' + current == start_term) or ('و' + start_term == current):
                return start_idx
            start_idx += 1
        return -1

    def get_correct_span(self, record_idx: int, sura_name: str, verse_number: str, original_verses: dict, normalized_verses: dict) -> str:
        extra_list = ['ۖ', ' ۗ', 'ۚ', 'ۗ']
        ...
        if (len(orig_tokens) - self.get_extra_count(orig, extra_list)) > len(in_text_tokens):
            normalized_orig = normalized_verses[sura_name][verse_number]
            start_idx = self.get_start_index(in_text_tokens[0], in_text_tokens[1], normalized_orig)
            if start_idx < 0:
                print('Error in get_correct_span')
                return orig
            st_str = '...' if start_idx > 0 else ''
            start_idx += self.get_extra_count(" ".join(orig_tokens[:start_idx]), extra_list)
    ...
    def get_original_str(self, original_verses: dict, normalized_verses: dict) -> str:
        ...
        for i in range(count - 1):
            result_str += self.get_correct_span(i, self.aya_name, str(self.start_idx + i), original_verses, normalized_verses) + '، '
        result_str += self.get_correct_span(count - 1, self.aya_name, str(self.start_idx + count - 1), original_verses, normalized_verses)
        result_str += '"' + end_str
        return result_str
